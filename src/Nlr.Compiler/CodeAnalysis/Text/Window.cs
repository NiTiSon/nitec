...
public sealed class Window
{
	...
	public Window(Source source)
	{
		_source = source;
		_textEnd = source.Length;
	...
	private bool MoreChars()
	{
		if (_offset >= _windowLength)
		{
			...
			_source.CopyTo(_basis + _windowLength,
				_window,
				_windowLength,
				amountToRead);
	...
	public void Reset(int position)
	{
		...
		else
		{
			...
			int amountToRead = int.Min(_source.Length, position + _window.Length) - position;
			amountToRead = Math.Max(amountToRead, 0);
			if (amountToRead > 0)
			{
				_source.CopyTo(position, _window, 0, amountToRead);
			}
